<template>
  <div class="relative min-h-screen w-full max-w-md mx-auto bg-slate-100 overflow-hidden">
    ...
    <button 
      ...
      :style="{ bottom: (sheetCollapsed ? '120px' : '380px'), transition: 'bottom 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' }"
      ...
      <div class="flex flex-col gap-4">
        ...
        <div v-else class="flex items-start gap-2 rounded-xl px-3 py-3 transition-colors text-left" :class="isInRadius ? 'bg-blue-50 text-blue-700' : 'bg-rose-50 text-rose-700'">
          ...
          <p class="text-[11px] font-medium leading-relaxed">
            {{ isLoadingLocation ? 'Memverifikasi lokasi GPS...' : (isInRadius ? `Anda berada di radius ${namaLokasiTerdekat} (${jarakUserKeKantor}m).` : `Diluar jangkauan area absensi (${jarakUserKeKantor}m).`) }}
          </p>
        </div>
...
import { ArrowLeft, MapPin, Fingerprint, ShieldAlert, LocateFixed } from 'lucide-vue-next'
...
onMounted(async () => {
  ...
  // Start GPS watch — mendapat posisi pertama secepat mungkin
  startGPSWatch()
})

/**
 * Start GPS watchPosition — langsung dapat posisi pertama yang tersedia.
 * Jauh lebih cepat daripada 2x getCurrentPosition + setTimeout.
 */
const startGPSWatch = () => {
  isLoadingLocation.value = true
  securityError.value = null

  // Bersihkan watcher sebelumnya jika ada
  if (watchId !== null) {
  ...
  watchId = navigator.geolocation.watchPosition(
    (pos) => {
      ...
      if (!audit.isValid) {
        ...
        // Stop watching on security failure
        if (watchId !== null) {
          navigator.geolocation.clearWatch(watchId)
          watchId = null
        }
      ...
      // Hanya pada pembacaan pertama: fit bounds ke area kantor + user
    ...
    { 
      ...
      maximumAge: 5000,     // Boleh pakai cache GPS 5 detik terakhir → instant pada load pertama
      timeout: 8000         // Timeout 8 detik jika GPS benar-benar tidak responsif
    }
  )
}

/**
 * Proses lokasi kantor terdekat berdasarkan posisi user.
 */
...
/**
 * Fit map bounds ke posisi user + kantor terdekat
 */
...
/**
 * Tombol Kalibrasi — force refresh GPS position tanpa cache
 */
const recalibrateLocation = () => {
  ...
  // Force fresh GPS read tanpa cache
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      // Restart watcher setelah kalibrasi
      startGPSWatch()
    },
    () => {
      isLoadingLocation.value = false
    ...
    { 
      enableHighAccuracy: true, 
      maximumAge: 0,       // Force fresh — TIDAK boleh pakai cache
      timeout: 10000 
    }
  )
}
